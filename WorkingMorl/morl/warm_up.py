...
def initialize_warm_up_batch(args, device):
    ...
    for weights in weights_batch:
        # Prepare base_kwargs for network configuration
        base_kwargs = {'layernorm': args.layernorm}
        
        # Add network configuration if available
        if hasattr(args, 'base_kwargs') and args.base_kwargs:
            base_kwargs.update(args.base_kwargs)
        
        # Add hidden_size if specified
