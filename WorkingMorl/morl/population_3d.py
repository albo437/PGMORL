...
def predict_hyperbolic(args, opt_graph, index, test_weights):
    ...
    for dim in range(M):
        ...
        # Handle case with insufficient training data
...
class Population:
    ...
    def prediction_guided_selection(self, args, iteration, ep, opt_graph, scalarization_template):
        ...
        for sample in self.sample_batch:
            ...
            if isinstance(weight_center, torch.Tensor):
                weight_center = weight_center.detach().numpy()
            weight_center = np.array(weight_center)
            ...
            for i in range(len(weight_indices)):
                ...
                if angle < np.pi / 4.0: # check if weight is close to the previous weight
                    duplicated = False
                    for succ in opt_graph.succ[sample.optgraph_id]:
                        w = safe_to_numpy(opt_graph.weights[succ])
                        w = w / np.sum(w)
        ...
        for _ in range(N):
            ...
            weight = safe_to_numpy(candidates[best_id]['weight'])
            scalarization.update_weights(weight / np.sum(weight))
