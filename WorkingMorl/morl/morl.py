...
def run(args):
    ...
    torch.set_default_dtype(torch.float64)  # Use float64 for precision
    ...
    # Initialize population with warm-up samples
    population.update(elite_batch)
        ...
        if args.selection_method == 'moead':
            ...
            for weights in weights_batch:
                ...
                for sample in population.sample_batch:
                    value = scalarization.evaluate(torch.tensor(sample.objs, dtype=torch.float64))
                    if value > best_value:
    ...
    # save ep policies & env_params (only if not disabled)
    if not args.no_save_models:
        ...
        print("Skipping model file saving (--no-save-models enabled)")
    
    # save all ep objectives (always save this)
