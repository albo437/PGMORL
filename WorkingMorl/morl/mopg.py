...
def evaluation(args, sample):
    ...
    with torch.no_grad():
        for eval_id in range(args.eval_num):
            ...
            ob, _ = eval_env.reset()  # Handle new Gym API (observation, info)
            done = False
            gamma = 1.0
            while not done:
                if args.ob_rms:
                    ob = np.clip((ob - ob_rms.mean) / np.sqrt(ob_rms.var + 1e-8), -10.0, 10.0)
                _, action, _, _ = policy.act(torch.tensor(ob, dtype=torch.float64).unsqueeze(0), None, None, deterministic=True)
                ob, _, terminated, truncated, info = eval_env.step(action)
                done = terminated or truncated  # Handle new Gym API
                objs += gamma * info['obj']
                if not args.raw:
                    gamma *= args.gamma
    eval_env.close()
...
def MOPG_worker(args, task_id, task, device, iteration, num_updates, start_time, results_queue, done_event):
    # Set dtype consistency for this worker process
    torch.set_default_dtype(torch.float64)
    
    # Register custom environments for this worker process
    import gym
    try:
        import environments.dummy_mo_env  # This will register MO-Dummy-v0
    except ImportError:
    ...
    for j in range(start_iter, final_iter):
        ...
        for step in range(args.num_steps):
            ...
            done = terminated | truncated  # Handle new Gym API with numpy OR
